...
async function createItem(config: any, type: string, item: any): Promise<any> {
  const responseObj: responseObjInterface = { success: true, message: `${type === 'areas' ? 'Área' : 'Sub-área'} criada com sucesso` };
  switch (type) {
    case 'areas':
      config.method = 'post';
...
async function updateItem({ fields, type, id }: { fields: object, type: string, id: number | undefined }) {
  const responseObj: responseObjInterface = { success: true, message: `${type === 'areas' ? 'Área' : 'Sub-área'} atualizada com sucesso` };
...
async function deleteItem(config: any, type: string, id: number | undefined): Promise<any> {
  const responseObj: responseObjInterface = { success: true, message: `${type === 'areas' ? 'Área' : 'Sub-área'} deletada com sucesso` };
  switch (type) {
    case 'areas':
