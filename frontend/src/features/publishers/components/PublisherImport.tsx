...
interface PublisherImportProps {
  ...
  onImport: (formData: FormData, type: 'journal' | 'conference') => Promise<void>;
}

export function PublisherImport({ isOpen, onOpenChange, onImport }: PublisherImportProps) {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-4">
                        ...
                        <div className='flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-muted-foreground/10'>
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-medium truncate">{form.watch('file').name}</p>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Button
                type="submit"
                className="w-full"
                disabled={!form.watch('file') || importStatus === 'uploading' || importStatus === 'success'}
              >
