...
export function useProductionData({
  ...
  const { data: studentsData } = useQuery<{ data: Student[] }>({
    queryKey: ["students", "full"],
    queryFn: () => studentService.fetchStudents({ paginate: "false" }),
    enabled: !!auth?.isAdmin,
  });
  const studentsList = useMemo(() => studentsData?.data || [], [studentsData]);

  const { data: rawData, isLoading } = useQuery<Production[], Error>({
    queryKey: ["productions", selectedProfessorId, filters],
    queryFn: () => {
      const params = {
        filter: {
          is_featured: filters.isFeatured === "featured" ? true : undefined,
        },
      };
      ...
      if (auth?.isAdmin && selectedProfessorId.startsWith("professor:")) {
  ...
  const filteredAndSortedProductions = useMemo(() => {
    ...
    if (filters.titulo.trim())
      result = result.filter((p) =>
        p.title.toLowerCase().includes(filters.titulo.toLowerCase()),
      );
    ...
    if (filters.qualis !== "all")
      result = result.filter((p) => {
        const q = qualisList.find(
    ...
    result.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        case "titulo":
          ...
          break;
        case "local":
          aVal = (a.publisher?.name || "").toLowerCase();
          bVal = (b.publisher?.name || "").toLowerCase();
          break;
        case "year":
          aVal = a.year;
          bVal = b.year;
          break;
        case "tipo":
          aVal = a.publisher_type || "";
          bVal = b.publisher_type || "";
          break;
        case "origem":
          aVal = a.source || "";
          bVal = b.source || "";
  ...
  const filteredScore = useMemo(
    () =>
      filteredAndSortedProductions.reduce(
        (acc, p) =>
          acc +
          (qualisMap.get(p.publisher?.stratum_qualis?.id ?? -1)?.score ?? 0),
        0,
      ),
