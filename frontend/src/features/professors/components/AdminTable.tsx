...
export function AdminTable() {
  ...
  const renderMobileCard = (row: Row<AdminRequest>) => {
    const admin = row.original;
    const isProcessed = ['approved', 'rejected'].includes(admin.admin_status ?? 'notfound');
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium">
              {statusLabels[admin.admin_status as keyof typeof statusLabels || 'notfound']}
            </p>
          </div>
          <div>
            <Label className="text-xs text-muted-foreground">Data</Label>
            <p className="font-medium">{formatarData(admin.admin_requested_at)}</p>
          </div>
        </div>
        ...
        {isProcessed ? (
          <div>
            ...
            <p className="font-medium">{admin.approver?.name || 'Não Encontrado'}</p>
          </div>
        ) : (
            <CardFooter className="flex gap-2 pt-2 border-t p-0 mt-2">
              <Button
                ...
                onClick={() => approveMutation.mutate(admin.id)}
              >
                {approveMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4 mr-2" />}
                Aprovar
              </Button>
              <Button
                variant="destructive"
                ...
                onClick={() => rejectMutation.mutate(admin.id)}
              >
                {rejectMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4 mr-2" />}
                Rejeitar
              </Button>
          </CardFooter>
        )}
