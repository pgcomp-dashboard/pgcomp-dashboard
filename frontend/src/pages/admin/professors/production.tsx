...
export default function ProfessorProductionsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const rawData = await productionService.getUserProductions(
        Number(professorId),
      );
      console.log(rawData);
  ...
  const filteredAndSortedProductions = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "pontuacao":
          aValue =
            qualisList.find((q) => q.id === a.stratum_qualis_id)?.score || 0;
          bValue =
            qualisList.find((q) => q.id === b.stratum_qualis_id)?.score || 0;
          break;
        case "titulo":
          aValue = a.title || "";
          bValue = b.title || "";
  ...
  async function onSubmit(values: z.infer<typeof updateProductionFormSchema>) {
    ...
    try {
      ...
      let response;
      if (isCreationMode) {
        response = await productionService.createUserProduction(
          Number(professorId),
          payload,
        );
      } else if (selectedProduction) {
        response = await productionService.updateUserProduction(
  ...
  async function handleXmlUpload() {
    ...
    try {
      const response = await productionService.uploadUserLattes(
        Number(professorId),
        formData,
      );
  ...
  if (isProductionsLoading || isProfessorLoading)
    return (
      <div className="flex items-center justify-center p-10">
        <Loader2 className="animate-spin mr-2" />
        {isProfessorLoading
          ? "Carregando dados do professor..."
          : `Carregando produções de ${professor?.name}...`}
      </div>
    );
  if (productionsError)
    return (
      <div className="text-red-500 flex items-center p-10">
        <AlertCircle className="mr-2" /> Erro ao carregar produções.{" "}
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Lista de {productions?.length} produções vinculadas ao docente{" "}
              {professor?.name}.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              ...
              onValueChange={(val) =>
                setSortConfig((prev) => ({ ...prev, key: val as any }))
              }
            >
          ...
          {filteredAndSortedProductions.length === 0 ? (
            ...
            <div className="flex flex-wrap items-stretch gap-y-4">
              {filteredAndSortedProductions.map((prod, idx) => (
                <div
                  key={prod.id || idx}
                  className="flex w-full md:p-2 md:w-1/2"
                >
                  <Card className="bg-gray-50 flex flex-col w-full">
                    ...
                    <CardContent className="flex grow items-start md:items-end">
                      <div className="text-sm text-gray-600 space-y-1 wrap-break-word overflow-hidden">
                        ...
                        {prod.publisher && (
                          <p className="flex gap-1">
                            ...
                            <span>
                              {`${prod.publisher.name}${prod.publisher.initials ? ` (${prod.publisher.initials})` : ""} - ${prod.publisher.publisher_type}`}
                            </span>
                          </p>
                        )}
                        {prod.publisher?.stratum_qualis && (
                          <p>
                            <span className="font-medium">Qualis:</span>{" "}
                            {prod.publisher.stratum_qualis.code} (Score:{" "}
                            {prod.publisher.stratum_qualis.score})
                          </p>
                        )}
                      </div>
                    </CardContent>
                  ...
                  {/* <div
                  key={prod.id || idx}
                  className="rounded-lg border p-4 bg-gray-50 flex sm:flex-row gap-4 justify-between"
                >
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-[90vw] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {isCreationMode ? "Nova Produção" : "Editar Produção"}
            </DialogTitle>
            <DialogDescription>
              {isCreationMode
                ? "Preencha os dados da nova produção."
                : "Faça alterações na produção selecionada."}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isXmlOpen} onOpenChange={setIsXmlOpen}>
        <DialogContent className="max-w-[90vw] sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione o arquivo XML ou ZIP do currículo Lattes para importar
              as produções.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsXmlOpen(false)}>
              Cancelar
            </Button>
            <Button onClick={handleXmlUpload} disabled={!xmlFile}>
              Importar
            </Button>
          </DialogFooter>
        </DialogContent>
