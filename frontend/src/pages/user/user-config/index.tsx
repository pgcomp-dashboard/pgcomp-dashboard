      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Aqui você pode configurar suas informações no PGCOMP Dashboard.
          </p>
        </div>
      </div>
  ...
  const form = useForm<z.infer<typeof userConfigFormSchema>>({
    ...
    values: userInfo
      ? {
          ...
          registration:
            userInfo.type === "student" ? String(userInfo.registration) : "",
          siape: userInfo.type === "professor" ? userInfo.siape : 0,
          lattes_url:
            userInfo.type !== "manager" ? (userInfo.lattes_url ?? "") : "",
        }
      : undefined,
    resetOptions: {
...
function AskForAdminForm() {
  ...
  const {
    ...
    meta: {
      onError: (err: any) =>
        console.error("Erro ao carregar status de solicitação:", err),
    },
  });
  ...
  const statusConfig = {
    ...
    rejected: {
      ...
      description: null,
      buttonText: requestMutation.isPending
        ? "Solicitando..."
        : "Solicitar Novamente",
  ...
  const currentKey =
    status === "pending" || status === "rejected" ? status : "default";
